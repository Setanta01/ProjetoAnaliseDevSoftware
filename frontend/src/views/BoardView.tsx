  ...
  return (
    <main className="flex h-full flex-col overflow-hidden px-6 pt-6">
      ...
      {isLoading ? (
        ...
        <div className="mb-6 flex-1 overflow-y-auto rounded-lg border border-border bg-card"><Table><TableHeader><TableRow><TableHead>ID</TableHead><TableHead>Título</TableHead><TableHead>Tipo</TableHead><TableHead>Prioridade</TableHead><TableHead>Coluna</TableHead><TableHead>Pontos</TableHead></TableRow></TableHeader><TableBody>{sortedTasks.map((task) => <TableRow key={task.id} className="cursor-pointer border-b border-border last:border-b" onClick={() => void openCard(task.id)}><TableCell className="font-mono text-muted-foreground">{task.codigo ?? `#${task.id}`}</TableCell><TableCell className="font-semibold">{task.titulo}</TableCell><TableCell><Badge variant={task.tipo === 'BUG' ? 'danger' : 'neutral'}>{task.tipo === 'BUG' ? 'Bug' : 'Task'}</Badge></TableCell><TableCell><PriorityBadge prioridade={task.prioridade} /></TableCell><TableCell>{task.coluna_nome ?? columns.find((column) => column.id === task.coluna_id)?.nome ?? task.status}</TableCell><TableCell>{task.estimativa_consolidada ?? '-'}</TableCell></TableRow>)}</TableBody></Table></div>
      )}
    </main>
  )
}
