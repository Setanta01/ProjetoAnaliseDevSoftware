...
| `GET`    | `/cards/<id>/`              | `[MEMBRO]` | Detalhes completos do card. Quando `impedido=true`, inclui `impedimento_motivo` com a justificativa ativa.                                                                         |
| `PATCH`  | `/cards/<id>/`              | `[MEMBRO]` | Atualiza campos, move `coluna_id` ou altera responsavel. Gerente edita cards gerais; QA pode editar BUG e mover card aprovado de `Review` para `Done`. `sprint_id=null` move para backlog e limpa dados especificos da sprint. Cards no backlog só aceitam dados de sugestao; responsavel, prazo, estimativa, coluna, comentarios, checklist, anexos e validacao ficam bloqueados ate entrar em sprint. `due_date="SPRINT_ATUAL"` atrela entrega à sprint atual; `due_date=null` remove esse vínculo. |
...
| `POST`   | `/cards/<id>/impedimento/` | `[RESP/GER]` | **Payload:** `comentario`. Marca card como impedido e usa o comentario como motivo ativo. |
| `DELETE` | `/cards/<id>/impedimento/` | `[RESP/GER]` | Remove o impedimento do card e deixa de expor o motivo ativo.                  |
